...
`v0.9.0-rc3` for the next aligned validation tag.
...
- `v0.9.0-rc3` is recommended next so target Linux+bwrap evidence can be rerun against a tag that includes the diagnostics and runtime bundle fixes.
...
- Release gate remains NO_GO until current-head CI, real scanner, and external audit anchor or accepted risk evidence are archived.
...
- A target VM previously passed the bwrap backend probe but failed worker/runtime validation. Post-RC2 diagnostics fixed that path; copied VM evidence now shows target Linux+bwrap `production-required` validation passing, but it should be rerun on an aligned `v0.9.0-rc3` checkout.
...
- Release Gate: `NO_GO` due to `ci.matrix`, `scanner.configured`, `doctor.production_blocking`, `audit.verify`, and `scanner.policy`.
